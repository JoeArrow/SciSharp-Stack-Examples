...
namespace JWTensorflowNET.BasicModels
{
    // ----------------------------------------------------
    /// <summary>
    ///     LogisticRegressionEager Description
    /// </summary>

    public class LogisticRegressionEager : absSciSharpBase, IJWTensorBase
    {
        int batch_size = 256;
        ...
        public void RunEagerMode()
        {
            ...
            Func<Tensor, Tensor> logistic_regression = x => tf.nn.softmax(tf.matmul(x, W) + b);
            ...
            {
                // --------------------------------------
                // Wrap computation inside a GradientTape
            ...
            using(var sess = tf.Session())
            {
                ...
                foreach(var epoch in range(training_epochs))
                {
                    ...
                    foreach(var i in range(total_batch))
                    {
                        ...
                        (_, float c) = sess.run((loss, cost), (x, batch_xs), (y, batch_ys));
