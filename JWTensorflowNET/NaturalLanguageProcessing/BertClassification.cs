...
namespace JWTensorflowNET.NaturalLanguageProcessing
{
    // ----------------------------------------------------
    /// <summary>
    ///     BertClassification Description
    /// </summary>

    public class BertClassification : absSciSharpBase, IJWTensorBase
    {
        int epoch = 10;
        int batch_size = 32;
        int num_classes = 2;
        ...
        public bool Run(IReq? req = null)
        {
            ...
            model.add(new BertEmbedding.BertMainLayer(config));

            if(File.Exists(pretrained_weight_path)) { model.load_weights(pretrained_weight_path); }

            model.add(keras.layers.Dense(num_classes));

            model.compile(optimizer: keras.optimizers.AdamW(learning_rate,  
                                                            weight_decay: 0.01f, 
                                                            no_decay_params: new List<string> { "gamma", "beta" }),
