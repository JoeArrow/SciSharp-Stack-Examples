...
namespace JWTensorflowNET.NeuralNetworks
{
    // ----------------------------------------------------
    /// <summary>
    ///     NeuralNetXor Description
    /// </summary>

    public class NeuralNetXor : absSciSharpBase, IJWTensorBase
    {
        ...
        private float RunWithImportedGraph()
        {
            ...
            while(step < num_steps)
            {
                // ----------------
                // original python:
                //_, step, loss_value = sess.run([train_op, gs, loss], feed_dict={features: xy, labels: y_})
        ...
        private (Operation, Tensor, Tensor) make_graph(Tensor features, Tensor labels, int num_hidden = 8)
        {
            ...
            var output_weights = tf.Variable(tf.truncated_normal(new[] { num_hidden, 1 },
                                                                 seed: 17,
