...
namespace JWTensorflowNET.NeuralNetworks
{
    // ----------------------------------------------------
    /// <summary>
    ///     FuelEfficiencyPrediction Description
    /// </summary>

    public class FuelEfficiencyPrediction : absSciSharpBase, IJWTensorBase
    {
        ...
        public override void PrepareData()
        {
            ...
            var dataset = pd.read_csv(url,
                names: new[] { "MPG", "Cylinders", "Displacement", "Horsepower", "Weight", "Acceleration", "Model Year", "Origin" },
                sep: ' ',
                na_values: '?',
                comment: '\t',
            ...
            var history = horsepower_model.fit(train_features["Horsepower"], 
                                               train_labels, 
                                               epochs: 100,
            ...
            var results = horsepower_model.evaluate(test_features["Horsepower"], test_labels, verbose: 1);

            // --------------------------------------
            // Linear regression with multiple inputs

            var linear_model = keras.Sequential(normalizer, layers.Dense(units: 1));

            linear_model.compile(optimizer: tf.keras.optimizers.Adam(learning_rate: 0.1f),
                                 loss: tf.keras.losses.MeanAbsoluteError());
            ...
            dnn_model.compile(optimizer: tf.keras.optimizers.Adam(learning_rate: 0.001f),
                              loss: tf.keras.losses.MeanAbsoluteError());
